...
def Read_File(file_path, dedisperse = True):
    File = psrchive.Archive.load(file_path)
    File.tscrunch()
    if dedisperse == True:
        File.dedisperse()
    File.pscrunch()
...
def cleanRFI_Narrowband(data, cutoff, cutoff_is_rms = False):
    ...
    mad_median = stats.median_abs_deviation(median_arr, nan_policy='omit')
    mad_std = stats.median_abs_deviation(std_arr, nan_policy='omit')
    mad_kurtosis = stats.median_abs_deviation(kurtosis_arr, nan_policy='omit')
    mad_skew = stats.median_abs_deviation(skew_arr, nan_policy='omit')
    ...
    #Here we use the mean absolute deviation
    if (median_median + thresh*mad_median) > cutoff or (median_median + thresh*mad_median) < -cutoff:
        cutoff_median = median_median + thresh*mad_median
    if (median_std + thresh*mad_std) > cutoff or (median_std + thresh*mad_std) < -cutoff:
        cutoff_std = median_std + thresh*mad_std
    if (median_kurtosis + thresh*mad_kurtosis) > cutoff or (median_kurtosis + thresh*mad_kurtosis) < -cutoff:
        cutoff_kurtosis = median_kurtosis + thresh*mad_kurtosis
    if (median_skew + thresh*mad_skew) > cutoff or (median_skew + thresh*mad_skew) < -cutoff:
        cutoff_skew = median_skew + thresh*mad_skew
    
    #Here we use the standard deviation
    #if (median_median + thresh*std_median) > cutoff or (median_median + thresh*std_median) < -cutoff:
    #    cutoff_median = median_median + thresh*std_median
    #if (median_std + thresh*std_std) > cutoff or (median_std + thresh*std_std) < -cutoff:
    #    cutoff_std = median_std + thresh*std_std
    #if (median_kurtosis + thresh*std_kurtosis) > cutoff or (median_kurtosis + thresh*std_kurtosis) < -cutoff:
    #    cutoff_kurtosis = median_kurtosis + thresh*std_kurtosis
    #if (median_skew + thresh*std_skew) > cutoff or (median_skew + thresh*std_skew) < -cutoff:
    #    cutoff_skew = median_skew + thresh*std_skew
...
def plot_statistics(data, file_path, freq_mhz, numsubint, freq_domain_median, freq_domain_std, freq_domain_kurtosis, \
                    ...
                    time_domain_kurtosis, time_domain_skew, RFI_removed, SNR, on_pulse_start, on_pulse_end, folded, plot_title_dedisp, plot_title_disp):
    ...
    ax4.axvline(x=on_pulse_start)
    ax4.axvline(x=on_pulse_end)    
...
def clean_and_plot(file_path, zapped_file, threshold_dedisp, threshold_disp, on_pulse_start, on_pulse_end, folded = False, \
    ...
    if plot_compact_initial_stats == True:
        ...
        plot_statistics(data_dedisp, file_path, data_disp, 512, median_arr_ini, std_arr_ini, kurtosis_arr_ini, skew_arr_ini, \
                        ...
                        snr_ini, on_pulse_start, on_pulse_end, folded, plot_title_dedisp = 'De-Dispersed Un-Cleaned Data', \
    ...
    if plot_compact_stats == True:
        plot_statistics(result_array, file_path, freq_mhz, bin_num, median_arr, std_arr, kurtosis_arr, skew_arr, \
                        median_arr_disp, std_arr_disp, kurtosis_arr_disp, skew_arr_disp, nan_percent, snr, on_pulse_start, on_pulse_end, folded,\
